...
/**
 * Entity de `public.contract_items` — generado desde prod en vivo (`hklompkypzqtglprfobu`, MCP Supabase, 2026-08-22). 1118 filas · RLS on.
 ...
 * Referenciada por FK desde 6 tabla(s): contract_amendment_items, invoice_items, invoice_items_legacy_match, quantities, revenue_schedule_monthly, sapira_quantity_imports.
...
export class ContractItem {
	...
	@Column({ type: 'date', comment: 'Fecha de inicio para la prestación del servicio/producto y devengo de ingresos', nullable: true })
	start_date?: Date;
	...
	@Column({ type: 'uuid', comment: 'Referencia al item original (para items negativos de DOWNSELL/CHURN)', nullable: true })
	related_item_id?: string;

	/** Categorización libre del item (ej: Licencias, Servicios, Hardware) */
	@Column({ type: 'varchar', length: 64, comment: 'Categorización libre del item (ej: Licencias, Servicios, Hardware)', nullable: true })
	item_type?: string;

	/** Unidad de medida del producto/servicio (UND, PERIODOS, etc.) */
	@Column({ type: 'varchar', length: 32, comment: 'Unidad de medida del producto/servicio (UND, PERIODOS, etc.)', nullable: true })
	...
	@Column({
		...
		precision: 18,
		scale: 6,
		comment: 'Precio unitario base (opcional). Si existe, price = unit_price × quantity × term_months',
		nullable: true,
	})
	unit_price?: number;

	/** Cantidad del producto/servicio en el contrato */
	@Column({ type: 'numeric', precision: 18, scale: 6, comment: 'Cantidad del producto/servicio en el contrato', nullable: true })
	quantity?: number;

	/** Código de cuenta contable (texto libre, opcional) */
	@Column({ type: 'varchar', length: 128, comment: 'Código de cuenta contable (texto libre, opcional)', nullable: true })
	account?: string;

	/** Campos personalizados definidos por el usuario en formato JSONB */
	@Column({ type: 'jsonb', comment: 'Campos personalizados definidos por el usuario en formato JSONB', nullable: true, default: '{}' })
	custom_fields?: any;

	/** Fecha efectiva de churn para este item. Si existe, tiene prioridad sobre end_date para cálculos. */
	@Column({
		type: 'date',
		comment: 'Fecha efectiva de churn para este item. Si existe, tiene prioridad sobre end_date para cálculos.',
		nullable: true,
	})
	churn_date?: Date;
	...
	@Column({
		...
		precision: 18,
		scale: 2,
		comment: 'Monto mensual que se pierde por el churn. Calculado como final_price / term_months.',
		nullable: true,
	})
	...
	@Column({
		...
		precision: 18,
		scale: 2,
		comment: 'Precio mensual del item (final_price para recurrentes). INCLUYE descuentos aplicados. Usado para cálculo de MRR.',
		nullable: true,
	})
	...
	@Column({ type: 'boolean', comment: 'Indica si este item debe renovarse automáticamente al vencer', nullable: false, default: false })
	auto_renew: boolean;
	...
	@Column({ type: 'timestamp with time zone', comment: 'Timestamp de cuándo se ejecutó la última renovación automática', nullable: true })
	...
	@Column({ type: 'date', comment: 'Fecha de booking del item. Desde cuando el CMRR se registra en RSM. Si NULL, usa start_date.', nullable: true })
	booking_date?: Date;
