...
import { CONTRATOS_PROD_SNAPSHOT } from './contratos.prod-snapshot';

import * as modulo from './index';

/**
 * Verifica que los espejos del módulo `contratos` coinciden con el snapshot de prod (columnas + nullabilidad, PK,
 ...
 */
describe('Espejo contratos (TypeORM ↔ prod public)', () => {
	...
	it('mapea exactamente las tablas del snapshot (las que no tenían entity en el repo)', () => {
		expect(mirrorMetadatas.map((m) => m.tableName).sort()).toEqual(Object.keys(CONTRATOS_PROD_SNAPSHOT).sort());
	});
	...
	describe.each(Object.entries(CONTRATOS_PROD_SNAPSHOT))('%s', (table, expected) => {
