...
| `workflow_steps` (13, RLS on) | `workflow-step.espejo.ts` · `WorkflowStep` | 13 | `workflow_steps_pkey` (id) | — | — | — | `idx_workflow_steps_holding_id`, `idx_workflow_steps_order` | update_workflow_steps_updated_at · BEFORE UPDATE FOR EACH ROW → update_workflow_steps_updated_at() | 4 |
| `workflow_step_documents` (-1, RLS on) | `workflow-step-document.espejo.ts` · `WorkflowStepDocument` | 10 | `workflow_step_documents_pkey` (id) | — | — | `fk_wsd_contract` → contracts (CASCADE)<br>`fk_wsd_holding` → company_holdings (RESTRICT)<br>`fk_wsd_step` → workflow_steps (SET NULL)<br>`fk_wsd_uploaded_by` → users (SET NULL)<br>`workflow_step_documents_contract_id_fkey` → contracts (CASCADE)<br>`workflow_step_documents_holding_id_fkey` → company_holdings (RESTRICT)<br>`workflow_step_documents_uploaded_by_fkey` → users<br>`workflow_step_documents_workflow_step_id_fkey` → workflow_steps (CASCADE) | `idx_wsd_contract`, `idx_wsd_holding`, `idx_wsd_step`, `idx_wsd_uploaded_at` (expresión, no declarado) | — | 4 |
| `contract_clauses` (18, RLS on) | `contract-claus.espejo.ts` · `ContractClaus` | 7 | `contract_clauses_pkey` (id) | — | — | `contract_clauses_company_id_fkey` → companies<br>`fk_contract_clauses_holding_id` → company_holdings (CASCADE) | `idx_contract_clauses_holding_id` | — | 4 |
| `contract_templates` (-1, RLS on) | `contract-template.espejo.ts` · `ContractTemplate` | 6 | `contract_templates_pkey` (id) | — | — | `contract_templates_company_id_fkey` → companies<br>`fk_contract_templates_holding_id` → company_holdings (CASCADE) | `idx_contract_templates_holding_id` | — | 4 |
| `contract_documents` (-1, RLS on) | `contract-document.espejo.ts` · `ContractDocument` | 11 | `contract_documents_pkey` (id) | — | — | `contract_documents_contract_id_fkey` → contracts (CASCADE)<br>`contract_documents_holding_id_fkey` → company_holdings (CASCADE)<br>`contract_documents_uploaded_by_fkey` → users | `idx_contract_documents_contract_id`, `idx_contract_documents_holding_id` | — | 4 |
| `contract_notifications` (-1, RLS on) | `contract-notification.espejo.ts` · `ContractNotification` | 10 | `contract_notifications_pkey` (id) | — | `contract_notifications_notification_type_check` | `contract_notifications_contract_id_fkey` → contracts (CASCADE)<br>`contract_notifications_holding_id_fkey` → company_holdings<br>`contract_notifications_user_id_fkey` → users | `idx_contract_notifications_contract_id`, `idx_contract_notifications_created_at`, `idx_contract_notifications_is_read`, `idx_contract_notifications_user_id` | — | 3 |
...
| `contract_invoices` (4983, RLS on) | `contract-invoice.espejo.ts` · `ContractInvoice` | 17 | `contract_invoices_pkey` (id) | — | — | `contract_invoices_satisfied_by_legacy_id_fkey` → invoices_legacy (SET NULL)<br>`fk_contract_invoices_contract_id` → contracts (CASCADE)<br>`fk_contract_invoices_holding_id` → company_holdings (CASCADE) | `idx_contract_invoices_contract_id`, `idx_contract_invoices_date`, `idx_contract_invoices_holding_id`, `idx_contract_invoices_status` | — | 5 |
| `churn_reasons` (-1, RLS on) | `churn-reason.espejo.ts` · `ChurnReason` | 6 | `churn_reasons_pkey` (id) | `churn_reasons_holding_id_name_key` | — | — | `idx_churn_reasons_active`, `idx_churn_reasons_holding` | churn_reasons_set_updated_at · BEFORE UPDATE FOR EACH ROW → set_updated_at() | 4 |
