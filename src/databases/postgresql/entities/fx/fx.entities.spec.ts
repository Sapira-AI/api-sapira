...
import { FX_PROD_SNAPSHOT } from './fx.prod-snapshot';

import * as modulo from './index';

/**
 * Verifica que los espejos del módulo `fx` coinciden con el snapshot de prod (columnas + nullabilidad, PK,
 ...
 */
describe('Espejo fx (TypeORM ↔ prod public)', () => {
	...
	it('mapea exactamente las tablas del snapshot (las que no tenían entity en el repo)', () => {
		expect(mirrorMetadatas.map((m) => m.tableName).sort()).toEqual(Object.keys(FX_PROD_SNAPSHOT).sort());
	});
	...
	describe.each(Object.entries(FX_PROD_SNAPSHOT))('%s', (table, expected) => {
