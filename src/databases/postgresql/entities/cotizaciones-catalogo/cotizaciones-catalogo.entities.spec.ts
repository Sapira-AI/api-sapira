...
import { COTIZACIONES_CATALOGO_PROD_SNAPSHOT } from './cotizaciones-catalogo.prod-snapshot';
...
describe('Espejo cotizaciones-catalogo (TypeORM ↔ prod public)', () => {
	...
	it('mapea exactamente las tablas del snapshot (las que no tenían entity en el repo)', () => {
		expect(mirrorMetadatas.map((m) => m.tableName).sort()).toEqual(Object.keys(COTIZACIONES_CATALOGO_PROD_SNAPSHOT).sort());
	});
	...
	describe.each(Object.entries(COTIZACIONES_CATALOGO_PROD_SNAPSHOT))('%s', (table, expected) => {
