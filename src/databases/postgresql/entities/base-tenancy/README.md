...
| `role_permissions` (637, RLS on) | `role-permission.espejo.ts` · `RolePermission` | 3 | `role_permissions_pkey` (role_id, permission_id) | — | — | `fk_role_permissions_holding_id` → company_holdings (CASCADE)<br>`role_permissions_permission_id_fkey` → permissions<br>`role_permissions_role_id_fkey` → roles | `idx_role_permissions_holding_id`, `idx_role_permissions_permission_id`, `idx_role_permissions_role_id` | — | 5 |
...
| `user_view_preferences` (9, RLS on) | `user-view-preference.espejo.ts` · `UserViewPreference` | 9 | `user_view_preferences_pkey` (id) | `user_view_preferences_user_id_entity_type_view_name_key` | — | `user_view_preferences_user_id_fkey` → users (CASCADE) | `idx_user_view_prefs_one_default_per_entity` (UNIQUE, parcial), `idx_user_view_prefs_user_entity` | user_view_preferences_updated_at · BEFORE UPDATE FOR EACH ROW → update_user_view_preferences_updated_at() | 4 |
