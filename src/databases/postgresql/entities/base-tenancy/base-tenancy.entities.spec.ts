...
import { BASE_TENANCY_PROD_SNAPSHOT } from './base-tenancy.prod-snapshot';

import * as modulo from './index';

/**
 * Verifica que los espejos del módulo `base-tenancy` coinciden con el snapshot de prod (columnas + nullabilidad, PK,
 ...
 */
describe('Espejo base-tenancy (TypeORM ↔ prod public)', () => {
	...
	it('mapea exactamente las tablas del snapshot (las que no tenían entity en el repo)', () => {
		expect(mirrorMetadatas.map((m) => m.tableName).sort()).toEqual(Object.keys(BASE_TENANCY_PROD_SNAPSHOT).sort());
	});
	...
	describe.each(Object.entries(BASE_TENANCY_PROD_SNAPSHOT))('%s', (table, expected) => {
