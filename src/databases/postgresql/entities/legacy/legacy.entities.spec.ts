...
import { LEGACY_PROD_SNAPSHOT } from './legacy.prod-snapshot';

import * as modulo from './index';

/**
 * Verifica que los espejos del módulo `legacy` coinciden con el snapshot de prod (columnas + nullabilidad, PK,
 ...
 */
describe('Espejo legacy (TypeORM ↔ prod public)', () => {
	...
	it('mapea exactamente las tablas del snapshot (las que no tenían entity en el repo)', () => {
		expect(mirrorMetadatas.map((m) => m.tableName).sort()).toEqual(Object.keys(LEGACY_PROD_SNAPSHOT).sort());
	});
	...
	describe.each(Object.entries(LEGACY_PROD_SNAPSHOT))('%s', (table, expected) => {
