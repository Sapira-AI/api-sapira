...
/**
 * Espejo de `public.invoice_items_legacy` — generado desde prod en vivo (`hklompkypzqtglprfobu`, MCP Supabase, 2026-08-22). 11698 filas · RLS on.
 ...
 */
@Entity({ name: 'invoice_items_legacy', comment: 'Líneas de detalle de facturas legacy' })
...
export class InvoiceItemsLegacy {
	...
	@Column({ type: 'text', comment: 'Moneda del item de factura. Puede ser NULL y heredar de la factura padre.', nullable: true })
	currency?: string;
	...
	@Column({
		type: 'text',
		comment: 'Tipo de item normalizado desde contract_item durante reconciliación (fixed, variable, usage-based, etc.)',
		nullable: true,
	})
	item_type?: string;
	...
	@Column({
		type: 'text',
		comment: 'Unidad de medida normalizada desde contract_item durante reconciliación (license, hours, users, etc.)',
		nullable: true,
	})
	...
	@Column({
		type: 'text',
		comment: 'ID de la línea en Odoo (account.move.line.id). Usado para identificar y evitar duplicados de líneas importadas desde Odoo.',
		nullable: true,
	})
	odoo_line_id?: string;
