...
import { AUTOMATIZACIONES_IA_PROD_SNAPSHOT } from './automatizaciones-ia.prod-snapshot';
...
describe('Espejo automatizaciones-ia (TypeORM ↔ prod public)', () => {
	...
	it('mapea exactamente las tablas del snapshot (las que no tenían entity en el repo)', () => {
		expect(mirrorMetadatas.map((m) => m.tableName).sort()).toEqual(Object.keys(AUTOMATIZACIONES_IA_PROD_SNAPSHOT).sort());
	});
	...
	describe.each(Object.entries(AUTOMATIZACIONES_IA_PROD_SNAPSHOT))('%s', (table, expected) => {
