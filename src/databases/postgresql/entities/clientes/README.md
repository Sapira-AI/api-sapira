...
| `client_documents` (8, RLS on) | `client-document.espejo.ts` · `ClientDocument` | 6 | `client_documents_pkey` (id) | — | — | `client_documents_client_id_fkey` → clients (CASCADE)<br>`fk_client_documents_holding_id` → company_holdings (CASCADE) | `idx_client_documents_holding_id` | — | 1 |
...
| `company_bank_accounts` (9, RLS on) | `company-bank-account.espejo.ts` · `CompanyBankAccount` | 9 | `company_bank_accounts_pkey` (id) | — | — | `company_bank_accounts_company_id_fkey` → companies (CASCADE) | — | — | 4 |
