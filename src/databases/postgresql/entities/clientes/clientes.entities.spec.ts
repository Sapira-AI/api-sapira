...
import { CLIENTES_PROD_SNAPSHOT } from './clientes.prod-snapshot';

import * as modulo from './index';

/**
 * Verifica que los espejos del módulo `clientes` coinciden con el snapshot de prod (columnas + nullabilidad, PK,
 ...
 */
describe('Espejo clientes (TypeORM ↔ prod public)', () => {
	...
	it('mapea exactamente las tablas del snapshot (las que no tenían entity en el repo)', () => {
		expect(mirrorMetadatas.map((m) => m.tableName).sort()).toEqual(Object.keys(CLIENTES_PROD_SNAPSHOT).sort());
	});
	...
	describe.each(Object.entries(CLIENTES_PROD_SNAPSHOT))('%s', (table, expected) => {
