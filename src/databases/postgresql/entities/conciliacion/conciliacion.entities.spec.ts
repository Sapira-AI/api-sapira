...
import { CONCILIACION_PROD_SNAPSHOT } from './conciliacion.prod-snapshot';

import * as modulo from './index';

/**
 * Verifica que los espejos del módulo `conciliacion` coinciden con el snapshot de prod (columnas + nullabilidad, PK,
 ...
 */
describe('Espejo conciliacion (TypeORM ↔ prod public)', () => {
	...
	it('mapea exactamente las tablas del snapshot (las que no tenían entity en el repo)', () => {
		expect(mirrorMetadatas.map((m) => m.tableName).sort()).toEqual(Object.keys(CONCILIACION_PROD_SNAPSHOT).sort());
	});
	...
	describe.each(Object.entries(CONCILIACION_PROD_SNAPSHOT))('%s', (table, expected) => {
