...
import { INTEGRACIONES_ODOO_PROD_SNAPSHOT } from './integraciones-odoo.prod-snapshot';

import * as modulo from './index';

/**
 * Verifica que los espejos del módulo `integraciones/odoo` coinciden con el snapshot de prod (columnas + nullabilidad, PK,
 ...
 */
describe('Espejo integraciones/odoo (TypeORM ↔ prod public)', () => {
	...
	it('mapea exactamente las tablas del snapshot (las que no tenían entity en el repo)', () => {
		expect(mirrorMetadatas.map((m) => m.tableName).sort()).toEqual(Object.keys(INTEGRACIONES_ODOO_PROD_SNAPSHOT).sort());
	});
	...
	describe.each(Object.entries(INTEGRACIONES_ODOO_PROD_SNAPSHOT))('%s', (table, expected) => {
