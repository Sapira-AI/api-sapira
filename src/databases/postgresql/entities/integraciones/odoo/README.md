...
> **Actualización 2026-09-09 · `processing_status` de `odoo_invoices_stg` y `odoo_invoice_lines_stg`.** La tabla ya no lista la divergencia "default `create` vs DB `'pending'::text`": se resolvió por el lado de la DB. El default `'pending'` de prod no estaba en el CHECK de la propia columna (`create | update | processed | error`), así que un INSERT que la omitiera generaba una fila inválida (23514). La migración `1788953100000-AlignStagingProcessingStatusDefault` lleva el default de ambas tablas a `'create'` y las entities lo declaran igual. El resto de la fila (divergencias de `created_at`/`updated_at`) sigue vigente. Al regenerar este README con `scripts/espejo/generate-espejo.py` sobre un snapshot posterior a la migración, la divergencia no debería reaparecer.
