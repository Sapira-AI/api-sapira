...
import { FACTURACION_PROD_SNAPSHOT } from './facturacion.prod-snapshot';

import * as modulo from './index';

/**
 * Verifica que los espejos del módulo `facturacion` coinciden con el snapshot de prod (columnas + nullabilidad, PK,
 ...
 */
describe('Espejo facturacion (TypeORM ↔ prod public)', () => {
	...
	it('mapea exactamente las tablas del snapshot (las que no tenían entity en el repo)', () => {
		expect(mirrorMetadatas.map((m) => m.tableName).sort()).toEqual(Object.keys(FACTURACION_PROD_SNAPSHOT).sort());
	});
	...
	describe.each(Object.entries(FACTURACION_PROD_SNAPSHOT))('%s', (table, expected) => {
