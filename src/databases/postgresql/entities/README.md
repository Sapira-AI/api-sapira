...
2. **Las entities existentes no se tocan ni se duplican**: las 59 clases `@Entity` del repo (`src/modules/*/entities/*.entity.ts` + las 8 planas de esta carpeta + `base.entity.ts`) están en producción — `database.module.ts` las carga por glob `src/**/*.entity.ts` y cada módulo las registra en `forFeature`. Siguen exactamente igual, prendidas como estaban. Para cada tabla que ya tiene entity, el README de su módulo documenta **dónde está y su diferencia con prod** (columnas faltantes/sobrantes, tipo/nullable/default/length distintos, constraints e índices no declarados), calculada con la metadata TypeORM real de la entity. Inventario: `scripts/espejo/existing-entities.json` (55 de las 59 apuntan a tablas que existen en prod; las 4 `integration_salesforce_*` no).
...
Archivos de soporte en esta carpeta (no los carga la app): `espejo.index.ts` (barrel de todos los espejos) y `espejo.existing.ts` (reexport de las 59 entities existentes) — los usan solo los specs para construir la metadata completa con todos los destinos de FK.
