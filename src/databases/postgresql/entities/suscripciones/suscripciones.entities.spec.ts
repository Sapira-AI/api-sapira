...
import { SUSCRIPCIONES_PROD_SNAPSHOT } from './suscripciones.prod-snapshot';

import * as modulo from './index';

/**
 * Verifica que los espejos del módulo `suscripciones` coinciden con el snapshot de prod (columnas + nullabilidad, PK,
 ...
 */
describe('Espejo suscripciones (TypeORM ↔ prod public)', () => {
	...
	it('mapea exactamente las tablas del snapshot (las que no tenían entity en el repo)', () => {
		expect(mirrorMetadatas.map((m) => m.tableName).sort()).toEqual(Object.keys(SUSCRIPCIONES_PROD_SNAPSHOT).sort());
	});
	...
	describe.each(Object.entries(SUSCRIPCIONES_PROD_SNAPSHOT))('%s', (table, expected) => {
