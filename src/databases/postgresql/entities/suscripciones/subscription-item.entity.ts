...
/**
 * Entity de `public.subscription_items` — generado desde prod en vivo (`hklompkypzqtglprfobu`, MCP Supabase, 2026-08-22). 458 filas · RLS on.
 ...
 */
@Entity({ name: 'subscription_items', comment: 'Items/líneas de una suscripción externa con pricing y producto mapeado a Sapira.' })
...
export class SubscriptionItem {
	...
	@Column({ type: 'text', comment: 'ID del producto en Stripe (ej: prod_xxx). Usado para mapeo via stripe_product_mappings.', nullable: true })
	stripe_product_id?: string;

	/** ID del precio/plan en Stripe (ej: pay_per_vehicle_2025). */
	@Column({ type: 'text', comment: 'ID del precio/plan en Stripe (ej: pay_per_vehicle_2025).', nullable: true })
	...
	@Column({ type: 'text', comment: 'Tipo fijo para filtros y análisis MRR (ej: Digital). Desde Master Data.', nullable: true, default: 'Digital' })
	item_type?: string;
	...
	@Column({
		...
		precision: 18,
		scale: 6,
		comment: 'Precio unitario en la moneda de la suscripción. Ya convertido de centavos para Stripe (÷100).',
		nullable: true,
		default: 0,
	})
	unit_price?: number;
	...
	@Column({ type: 'numeric', precision: 18, scale: 2, comment: 'Calculado: quantity × unit_price', nullable: true, default: 0 })
