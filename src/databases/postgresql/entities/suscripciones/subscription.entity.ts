...
/**
 * Espejo de `public.subscriptions` — generado desde prod en vivo (`hklompkypzqtglprfobu`, MCP Supabase, 2026-08-22). 435 filas · RLS on.
...
@Entity({
	name: 'subscriptions',
	comment: 'Suscripciones externas (Stripe, Chargebee, etc.) que no siguen el modelo de contratos a plazo fijo. Read-only desde Sapira.',
})
...
export class Subscription {
	...
	@Column({ type: 'text', comment: 'ID de la suscripción en el sistema externo (ej: sub_xxx para Stripe)', nullable: false })
	external_id: string;

	/** Sistema origen: stripe, chargebee, etc. Extensible para futuras integraciones. */
	@Column({
		type: 'text',
		comment: 'Sistema origen: stripe, chargebee, etc. Extensible para futuras integraciones.',
		nullable: false,
		default: 'stripe',
	})
	source: string;
	...
	@Column({
		...
		precision: 18,
		scale: 2,
		comment: 'Suma de monthly_amount de todos los subscription_items activos',
		nullable: true,
		default: 0,
	})
	...
	@Column({
		...
		precision: 18,
		scale: 6,
		comment: 'Tipo de cambio de currency a system_currency. Default 1 cuando son iguales.',
		nullable: true,
		default: 1,
	})
	fx_to_system?: number;
	...
	@Column({ type: 'text', comment: 'Campo libre editable por usuario de Sapira (único campo editable)', nullable: true })
	notes?: string;
