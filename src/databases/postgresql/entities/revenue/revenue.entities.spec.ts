...
import { REVENUE_PROD_SNAPSHOT } from './revenue.prod-snapshot';

import * as modulo from './index';

/**
 * Verifica que los espejos del módulo `revenue` coinciden con el snapshot de prod (columnas + nullabilidad, PK,
 ...
 */
describe('Espejo revenue (TypeORM ↔ prod public)', () => {
	...
	it('mapea exactamente las tablas del snapshot (las que no tenían entity en el repo)', () => {
		expect(mirrorMetadatas.map((m) => m.tableName).sort()).toEqual(Object.keys(REVENUE_PROD_SNAPSHOT).sort());
	});
	...
	describe.each(Object.entries(REVENUE_PROD_SNAPSHOT))('%s', (table, expected) => {
