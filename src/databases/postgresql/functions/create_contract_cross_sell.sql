CREATE OR REPLACE FUNCTION public.create_contract_cross_sell(p_contract_id uuid, p_items jsonb, p_effective_date date, p_reason text DEFAULT NULL::text, p_metadata jsonb DEFAULT '{}'::jsonb, p_approval_required boolean DEFAULT false)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
  ...
  IF p_items IS NOT NULL THEN
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
      ...
      v_items_summary := v_items_summary || jsonb_build_array(
        jsonb_build_object(
          'product_name', v_item->'item_metadata'->>'product_name',
          'price', v_item->'item_metadata'->>'price'
        )
      );
    END LOOP;
  END IF;
...
COMMENT ON FUNCTION public."create_contract_cross_sell"(p_contract_id uuid, p_items jsonb, p_effective_date date, p_reason text, p_metadata jsonb, p_approval_required boolean) IS 'Crea un amendment de tipo CROSS_SELL. Si approval_required=false, se auto-aprueba y registra en historial.';
