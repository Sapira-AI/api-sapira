...
export class ClientEntitiesController {
	...
	async list(@Query() query: QueryClientEntitiesDto) {
		return await this.directory.listEntities(query.holding_id, {
			page: query.page,
			limit: query.limit,
			search: query.search,
	...
	async assign(@Body() body: AssignClientEntitiesDto) {
		return await this.directory.assignEntities(body.holding_id, body.client_id, body.entity_ids, body.make_primary_if_none ?? true);
	}
	...
	@ApiQuery({ name: 'holding_id', type: String, required: true })
	async getDetail(@Param('id', new ParseUUIDPipe()) id: string, @Query('holding_id', new ParseUUIDPipe()) holdingId: string) {
	...
	@ApiParam({ name: 'id', type: String })
	async getInvoices(@Param('id', new ParseUUIDPipe()) id: string, @Query() query: QueryEntityInvoicesDto) {
		return await this.entityMetrics.getInvoices(id, query.holding_id, {
			page: query.page,
			limit: query.limit,
	...
	@ApiParam({ name: 'id', type: String })
	async update(@Param('id', new ParseUUIDPipe()) id: string, @Body() body: UpdateClientEntityDto) {
		const { holding_id: holdingId, allow_duplicate_tax_id: allowDuplicate, ...changes } = body;
